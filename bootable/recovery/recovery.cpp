...
#endif
#ifdef GMT_RECOVERY_MODE_FOTA_SUPPORT
#include <sys/wait.h>
#endif
#include <sys/types.h>
#include <time.h>
...
#ifdef GMT_RECOVERY_MODE_FOTA_SUPPORT
/*
 * GeneralMobile Tech FOTA Integration Functions
 */
#define GMT_PATH_MAX						1024
#define GMT_DEFAULT_DP_PARTTION		"/data"
#define GMT_DEFAULT_DP_LOCATION 	"/data/data/com.redbend.dmClient/files/vdm_update"
#define GMT_DEFAULT_RESULT_LOCATION 	"/data/data/com.redbend.dmClient/files/result"
...
//Ivan
...
#ifdef VOLTAGE_LIMIT_SUPPORT
    if (argc == 1)
    {
	int temp = 0;
	temp = get_ADC_channel(ADC_BAT_SEN, 5); 
	if (temp != -1) {		
		temp = (temp/5);
	} else {
		temp = 0;
	...
	{
	    ui->ShowText(true);	
	    ui->Print("BATTERY TOO LOW TO ENTER RECOVERY MODE!\n");
