...
#define VIDEO_TINNO_THREAD   0
#define VIDEO_TINNO_NOT_USE_THREAD   0

#if VIDEO_TINNO_THREAD
#include <utils/threads.h>

static pthread_t threadVideoTinno;
static sem_t semVidoThread, semVidoThreadBack, semVidoThreadEnd;

typedef enum {
    TN_VIDO_STATE_INIT,
    TN_VIDO_STATE_PROCESS,
    TN_VIDO_STATE_UNINIT,
} TinnoVidoState;
TinnoVidoState TNVidoState = TN_VIDO_STATE_UNINIT;

MVOID *TinnoVidoThread(MVOID *arg);
ImgBufQueNode TNVidoProcessNode;
ImgBufQueNode VidoNULLNode;

MVOID TNVideoProcess(TinnoVidoState state)
{
    MY_LOGD("Tinno preview)");
    if(TNVidoProcessNode.getImgBuf() != NULL)
    {
        MUINT8 *p = (MUINT8*)TNVidoProcessNode.getImgBuf()->getVirAddr();
        MUINT8* end_p = (MUINT8*)TNVidoProcessNode.getImgBuf()->getVirAddr()+ TNVidoProcessNode.getImgBuf()->getImgWidth()*3*30/2;
        while((p<(end_p)) )
        {
            *p=0;
            p++;
...
MVOID *TinnoVidoThread(MVOID *arg)
{
    ::prctl(PR_SET_NAME,"TinnoVideoThread", 0, 0, 0);   // Give this thread a name.
    ...
    MBOOL   ret = MTRUE;
    //
    MY_LOGD("[TinnoVideoThread] tid: %d.", gettid());
    while (TNVidoState != TN_VIDO_STATE_UNINIT)
    {
        ::sem_wait(&semVidoThread);
        MY_LOGD("[TinnoVideoThread] Got semVidoThread. TNVidoState: %d.", TNVidoState);
        if(TNVidoProcessNode.getImgBuf() != NULL){
            TNVideoProcess(TNVidoState);
        }
        ::sem_post(&semVidoThreadBack);
    }
   ::sem_post(&semVidoThreadEnd);

   TNVidoProcessNode = VidoNULLNode;

   MY_LOGD("[TinnoVideoThread] - X.");
...
#if VIDEO_TINNO_THREAD
    ::sem_init(&semVidoThread, 0, 0);
    ::sem_init(&semVidoThreadBack, 0, 0);
    ::sem_init(&semVidoThreadEnd, 0, 0);
    pthread_create(&threadVideoTinno, NULL, TinnoVidoThread, NULL);
    TNVidoState = TN_VIDO_STATE_INIT;
    MY_LOGD("[init] TinnoVidoThread: %d.", threadVideoTinno);
#endif
...
#if VIDEO_TINNO_THREAD
    MY_LOGD("[Stop start] TinnoVidoThread: %d.", threadVideoTinno);
    TNVidoState = TN_VIDO_STATE_UNINIT;
    MY_LOGD("[Stop] TinnoVidoThread: %d.", threadVideoTinno);
#endif
...
#if VIDEO_TINNO_THREAD
        MY_LOGD("Tinno start");
        TNVidoState = TN_VIDO_STATE_PROCESS;
        TNVidoProcessNode = dispNode;
        ::sem_post(&semVidoThread);
        ::sem_wait(&semVidoThreadBack);
        MY_LOGD("Tinno END");
#endif
