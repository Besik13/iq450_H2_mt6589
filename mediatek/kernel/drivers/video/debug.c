...
static void process_dbg_opt(const char *opt)
{
  ...
  //edit by Magnum 2013-3-27  add timing tuning
   else if (0 == strncmp(opt, "timing:", 7))
    {
    	extern LCM_PARAMS * lcm_params;
	printk("MTK71050 timing tuning \n");
        if (0 == strncmp(opt + 7, "da_hs_exit=", 11))
        {
            char *p;
	    unsigned int temp_data;
	    p= (char *)opt+11+7;
	    temp_data = (unsigned int) simple_strtoul(p, &p,10);
	   lcm_params->dsi.CLK_HS_EXIT = temp_data;
	   printk("MTK71050 CLK_HS_EXIT = %d read data= %d \n",lcm_params->dsi.CLK_HS_EXIT,temp_data);
        }
        else if (0 == strncmp(opt + 7, "ta_sure=", 8 ))
        {
            char *p;
	    unsigned int temp_data;
	    p= (char *)opt+8+7;
	    temp_data = (unsigned int) simple_strtoul(p, &p,10);
	   lcm_params->dsi.TA_SURE= temp_data;
	   printk("MTK71050 CLK_HS_EXIT = %d read data= %d \n",lcm_params->dsi.TA_SURE,temp_data);
        }
	else if (0 == strncmp(opt + 7, "ta_sack=", 8 ))
        {
            char *p;
	    unsigned int temp_data;
	    p= (char *)opt+8+7;
	    temp_data = (unsigned int) simple_strtoul(p, &p,10);
	   lcm_params->dsi.TA_SACK= temp_data;
	   printk("MTK71050 CLK_HS_EXIT = %d read data= %d \n",lcm_params->dsi.TA_SACK,temp_data);
        }
	else if (0 == strncmp(opt + 7, "ta_get=", 7 ))
        {
            char *p;
	    unsigned int temp_data;
	    p= (char *)opt+7+7;
	    temp_data = (unsigned int) simple_strtoul(p, &p,10);
	   lcm_params->dsi.TA_GET= temp_data;
	   printk("MTK71050 CLK_HS_EXIT = %d read data= %d \n",lcm_params->dsi.TA_GET,temp_data);
        }
	else if (0 == strncmp(opt + 7, "ta_go=", 6 ))
        {
            char *p;
	    unsigned int temp_data;
	    p= (char *)opt+6+7;
	    temp_data = (unsigned int) simple_strtoul(p, &p,10);
	   lcm_params->dsi.TA_GO= temp_data;
	   printk("MTK71050 CLK_HS_EXIT = %d read data= %d \n",lcm_params->dsi.TA_GO,temp_data);
        }
	else if (0 == strncmp(opt + 7, "lpx=", 4 ))
        {
            char *p;
	    unsigned int temp_data;
	    p= (char *)opt+4+7;
	    temp_data = (unsigned int) simple_strtoul(p, &p,10);
	   lcm_params->dsi.LPX= temp_data;
	   printk("MTK71050 CLK_HS_EXIT = %d read data= %d \n",lcm_params->dsi.LPX,temp_data);
        }
	else if (0 == strncmp(opt + 7, "clock=", 6 ))
        {
            char *p;
	    unsigned int temp_data;
	    p= (char *)opt+6+7;
	    temp_data = (unsigned int) simple_strtoul(p, &p,10);
	   lcm_params->dsi.PLL_CLOCK= temp_data;
	   printk("MTK71050 CLK_HS_EXIT = %d read data= %d \n",lcm_params->dsi.PLL_CLOCK,temp_data);
        }
	extern DSI_STATUS DSI_Reset_CLK(void);
	DSI_Reset_CLK();
    }
    else
	{
