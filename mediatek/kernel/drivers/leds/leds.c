...
//edit by Magnum 2013-3-19  global variable bl_frequency_enum  define the  lcd-light frequence
//attention: if the PWM_config of cust lcd_backlight is bigger than 0, the init 
// value of bl_frequency_enum
 //edit by Magnum 2013-3-19
unsigned int bl_div = CLK_DIV8;// solve noisy from speaker && camera.
unsigned int bl_frequency_enum = PWM_CLK_NEW_MODE_BLOCK; 
...
static int backlight_set_pwm(int pwm_num, u32 level, u32 div, struct PWM_config *config_data)
{
	...
	if(config_data->div)
	{
		LEDS_DEBUG("[LED] Magnum conig_data > 0 0 0 0 0 0 0 0 \n");
		pwm_setting.clk_div = config_data->div;
		backlight_PWM_div = config_data->div;
	}
	else{
	//	LEDS_DEBUG("[LED] Magnum conig_data > 11111111 \n");
	...
	if(config_data->clock_source)
		...
		pwm_setting.clk_src = bl_frequency_enum;
		}
	//edit by Magnum 2013-4-24  get the true value of bl_div && bl_frequency_enum
	bl_div = pwm_setting.clk_div ;
	bl_frequency_enum = pwm_setting.clk_src;
	LEDS_DEBUG("[LED]  bl_div == %d,   bl_frequency_enum == %d \n",bl_div,bl_frequency_enum);
	...
	pwm_setting.PWM_MODE_FIFO_REGS.IDLE_VALUE = 0;
	pwm_setting.PWM_MODE_FIFO_REGS.GUARD_VALUE = 0;
	pwm_setting.PWM_MODE_FIFO_REGS.STOP_BITPOS_VALUE = 63;		//Ivan 31;
	pwm_setting.PWM_MODE_FIFO_REGS.GDURATION = 0;			//Ivan (pwm_setting.PWM_MODE_FIFO_REGS.HDURATION+1)*32 - 1;
	pwm_setting.PWM_MODE_FIFO_REGS.WAVE_NUM = 0;
	...
	}else if(level>32 && level <=64)
	{
		...
		level -= 32;
//Ivan		pwm_setting.PWM_MODE_FIFO_REGS.SEND_DATA0 = (1 << level) - 1 ;
...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
		...
		else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK0)
		{
			...
			if (level) 
			{
				...
				//upmu_set_rg_spk_div_pdn(0x01);
				//upmu_set_rg_spk_pwm_div_pdn(0x01);
				upmu_set_isink_dim0_duty(31);			//Ivan 0 ~ 31
				
				upmu_set_isinks_ch0_step(0x5);			//Ivan 0 ~ 5, 4 ~ 24mA
...
{
	LEDS_DEBUG("[LED]get backlight div value is :%d \n",bl_div);
	return sprintf(buf, "%u\n", bl_div);
}
...
static ssize_t store_div(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	unsigned int div = 0;
	size_t count = 0;
	LEDS_DEBUG("[LED]set backlight div start \n");
	...
	if(count == size)
	{
         if(div < 0 || (div > 7))
		{
            LEDS_DEBUG("[LED]set backlight div parameter error: %d[div:0~7]\n", div);
         //   return 0;    //this lead error that  repeated calls the function
         	return size;
		}
        
		if(bl_setting->mode == MT65XX_LED_MODE_PWM)
		{
            LEDS_DEBUG("[LED]set PWM backlight div OK: div=%d, duty=%d\n", div, bl_duty);
        ...
        {
            LEDS_DEBUG("[LED]set cust backlight div OK: div=%d, brightness=%d\n", div, bl_brightness);
	        ((cust_brightness_set)(bl_setting->data))(bl_brightness, div);
        }
        
	//	bl_div = div;    edit by Magnum 
	}
...
static ssize_t show_frequency(struct device *dev,struct device_attribute *attr, char *buf)
{
    if(bl_setting->mode == MT65XX_LED_MODE_PWM)
    {
        //edit by Magnum 2013-4-12
	if(bl_frequency_enum == PWM_CLK_NEW_MODE_BLOCK_DIV_BY_1625){
		bl_frequency =  32000/div_array[bl_div];
	}
	else if(bl_frequency_enum == PWM_CLK_NEW_MODE_BLOCK){
		bl_frequency = 52000000/div_array[bl_div];
	}
    }
...
//edit by Magnum 2013-4-12  just PWM_CLK_NEW_MODE_BLOCK && 
// PWM_CLK_NEW_MODE_BLOCK_DIV_BY_1625 modes can be setted, as our PWM is FIFO
static ssize_t store_frequency(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	char *pvalue = NULL;
	size_t count = 0;
	LEDS_DEBUG("[LED] set backlight  frequency start \n");
	bl_frequency_enum = simple_strtoul(buf,&pvalue,10);
	count = pvalue - buf;
	//LEDS_DEBUG("[LED] set backlight  count == %d, pvalue == %d , buf == %d \n",count,pvalue,buf);
	if (*pvalue && isspace(*pvalue))
		count++;
		
	if(count == size)
	{
		LEDS_DEBUG("[LED] set backlight  frequency == %d  \n",bl_frequency_enum);
	        if(bl_frequency_enum ==2 || (bl_frequency_enum == 3))
		{
		    	if(bl_setting->mode == MT65XX_LED_MODE_PWM)
			{
				if(bl_frequency_enum == PWM_CLK_NEW_MODE_BLOCK_DIV_BY_1625){
					LEDS_DEBUG("[LED] set PWM backlight  frequency OK: frequence == 32000\n");
				}
				else if(bl_frequency_enum == PWM_CLK_NEW_MODE_BLOCK){
					LEDS_DEBUG("[LED] set PWM backlight  frequency OK: frequence == 52000000\n" );
				}
				backlight_set_pwm(bl_setting->data, bl_duty, bl_div,&bl_setting->config_data);
			}           
		}
		else{
			LEDS_DEBUG("[LED] set backlight  frequency  parameter must be  PWM_CLK_NEW_MODE_BLOCK  or  PWM_CLK_NEW_MODE_BLOCK_DIV_BY_1625\n");
	            //	return 0;    //this lead error that  repeated calls the function
		}
